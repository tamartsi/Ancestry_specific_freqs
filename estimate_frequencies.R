...
estimate_frequencies <- function(allele_counts, prop_mat, confidence = 0.95, 
  ...
  ## compute the negative log likelihood function: different way
  nll2 <- function(freqs){
    binomial_by_anc <- matrix(NA, nrow = nrow(prop_mat), ncol = ncol(prop_mat))
    for (i in 1:ncol(binomial_by_anc)){
      binomial_by_anc[,i] <- dbinom(allele_counts, max_counts, freqs[i])*prop_mat[,i]
    }
    nll_by_obs <- log(rowSums(binomial_by_anc))
    return(-sum(nll_by_obs))		
  }
  ## need to compare which function is better: nll1 or nll2
